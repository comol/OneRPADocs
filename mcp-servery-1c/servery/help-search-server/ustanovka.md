...
В том числе так выглядит незагрузившаяся embedding-модель: в light-образе недоступен `EMBEDDING_API_BASE` или неверен `EMBEDDING_API_KEY`, в полном образе нет модели в кэше `HF_HOME`. Процесс при этом не завершается; причина — в поле `failure` ответа `/ready` и в журнале на шаге «initializing the embedding model».

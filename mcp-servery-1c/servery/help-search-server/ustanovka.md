...
Путь к папке может быть любым. Но очень важно разместить векторную БД в томе на диске - чтобы избежать повторной индексации при старте контейнера.
...
Если LM Studio запущено локально параметры доступа к ней можно не указывать. Сепвер подключится к ней автоматически.

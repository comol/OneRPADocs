...
| Переменная | Описание | По умолчанию |
|------------|----------|--------------|
| `RELEVANCE_MAX_VECTOR_DISTANCE` | Максимальная cosine-distance векторного кандидата; меньше — лучше совпадение | `0.35` |
| `RELEVANCE_MAX_LEXICAL_RANK` | Сколько первых кандидатов лексической дорожки допускается независимо от численного score | `2` |
...
Векторная дорожка проходит по distance, лексическая — по рангу. У лексической дорожки в текущем контракте отдельного score-порога нет. Старый `RELEVANCE_MIN_LEXICAL_SCORE` больше не читается: при его наличии сервер пишет предупреждение и требует перейти на `RELEVANCE_MAX_LEXICAL_RANK`.
...
Общие правила, форма payload каждого хука и готовые примеры: [Доработка MCP: система плагинов](../../sistema-pluginov/) и [справочник хуков HelpSearchServer](../../sistema-pluginov/spravochnik-hukov.md#helpsearchserver).

...
| `INDEXING_WORKERS` | Максимум параллельных батчей индексации; запросы к embedding API могут выполняться одновременно | `5` |
...
`INDEXING_WORKERS` ограничивает число одновременно обрабатываемых батчей, но не гарантирует ускорения пропорционально значению. Чтение и подготовка документов должны успевать снабжать пул работой; запись в индекс сериализована. В полном образе локальная модель тоже вызывается последовательно, чтобы избежать конфликтов токенизатора. При работе через API сам сервер допускает параллельные HTTP-запросы, но провайдер может ставить их в очередь.

После изменения переменной пересоздайте контейнер. В начале индексации журнал показывает фактическое `INDEXING_WORKERS` и предел батчей в обработке. Для проверки конкурентности API сопоставляйте одновременно начатые запросы в журнале MCP и очередь на стороне провайдера; одинаковое время двух прогонов само по себе не означает, что настройка проигнорирована.

...
| `detail_level` | string | `detailed` | `detailed` — тела документов целиком, `compact` — только начальные фрагменты |
| `cursor` | string | *(не задано)* | `next_cursor` предыдущего ответа — продолжить после уже выданных документов |

**Возврат**: JSON-объект с полями `schema_version`, `outcome`, `total`, `returned`, `truncated`, `next_cursor` и `results`. Каждый результат несёт `doc_id`, `citation` (`source`, `name`, `object_name`, `full_name`, `doc_type`), `snippets`, собственный флаг `truncated`, итоговый `score` и `lanes` — вклад каждой дорожки поиска (`null` для дорожки, которая этот документ не нашла).
...
| `detail_level` | string | `detailed` | `detailed` или `compact` |
...
Все инструменты отвечают одним JSON-объектом с полем `outcome`:
...
| Тег | Архитектура | Embedding |
|-----|-------------|-----------|
| `latest` | amd64 | Локальная модель в образе (sentence-transformers) или внешний API |
| `light` | amd64 | Только внешний OpenAI-совместимый API (без PyTorch, образ ~121 МБ) |
| `arm64` | arm64 | Локальная модель в образе или внешний API |

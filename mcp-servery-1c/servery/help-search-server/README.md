...
## Обновление beta от 23.09.2026

В корпус синтакс-помощника вернулись около 626 страниц, которые раньше отсекал фильтр служебных файлов архива: ОписаниеТипов, ЦветаСтиля, ШрифтыСтиля, WebЦвета.Серебряный, МенеджерПолученияЛицензии и другие (фильтр сравнивал подстроки в пути, и «Description» попадал под «script»). При первом старте на существующем индексе сервер один раз полностью переиндексирует справку в staging-поколении; до её окончания отвечает прежнее поколение. Для light-образа это время и обращения к embedding API, как при первичной индексации.

Если не загрузилась embedding-модель, сборка индекса теперь завершается штатным отказом: `/ready` отвечает `degraded` с причиной в поле `failure`. Раньше процесс оставался в `starting` без объяснения.
...
Также исправлена публикация MCP-схем на Python 3.10: у top_k, doc_type и max_chars сохраняются описания, диапазоны и перечисления допустимых значений. Имена инструментов, значения по умолчанию и поведение поиска не меняются.

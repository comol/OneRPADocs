...
| `EMBEDDING_QUERY_PREFIX` | Префикс, добавляемый к тексту запроса перед эмбеддингом. Если переменная пуста, а модель — Qwen3-Embedding, запрос идёт с инструкцией задачи Qwen3 (см. [Инструкция к запросу у Qwen3](../../embedding-modeli/vybor-modeli.md#инструкция-к-запросу-у-qwen3)); смена этой инструкции индекс не пересобирает | *(пусто)* |
...
Образ с исправленной portable-сборкой zvec использует новую версию FTS-профиля.
При первом запуске после обновления Templates строит новое поколение индекса
шаблонов из исходных данных. Старые FTS-блоки нельзя исправить одной заменой
библиотеки; переиндексация может повторно вызвать embedding-провайдер.

Профиль проектной памяти не меняется: у неё нет FTS-полей, поэтому только это
обновление не требует перестроения памяти. Дождитесь успешного ответа `/ready`.

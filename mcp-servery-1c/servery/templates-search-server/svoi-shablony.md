...
ИИ может добавлять шаблоны программно через инструмент `add_template`. Обычно шаблон сразу сохраняется и индексируется без перезапуска. Если ответ содержит `status: "index_pending"` и `stored: true`, строка уже сохранена, но индекс ещё догоняет её через outbox — не повторяйте вызов, чтобы не создать дубль.
...
ИИ вызовет `add_template`; при обычном успехе шаблон сразу доступен через `templatesearch`, а при `index_pending` станет доступен после фонового восстановления индекса.

### 2. Через веб-интерфейс

1. Откройте `http://localhost:8004/extend/` и войдите под учётной записью с разрешением `create`
...
Шаблоны, добавленные через MCP или веб-интерфейс, индексируются автоматически. Обычно они доступны сразу; состояние `index_pending` означает, что долговечная запись уже создана, а outbox завершит индексацию. Перезапуск и повторное добавление **не требуются**.

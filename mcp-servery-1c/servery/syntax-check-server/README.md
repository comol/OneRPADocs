...
{% hint style="warning" %}
В поставляемом `bsl-analyzer.toml` отключены `UnresolvedMethod`, `FieldNotFound` и `UnusedLocalMethod`. При проверке отдельного временного файла анализатор не видит контекст всей конфигурации, поэтому эти межмодульные диагностики давали ложные ошибки на типичном коде 1С. Сервер проверяет синтаксис и локальные правила, но не доказывает корректность разрешения методов и полей во всём проекте.
{% endhint %}

...
Для крупных выгрузок поднимите лимит открытых файлов до `65536`, как в примере:
значения `1024` может не хватить. Пример использует обычную сеть Docker bridge.
Начиная с beta `1.1.13-beta.20260914`, дочерний анализатор выбирает свободный
порт на `127.0.0.1` при запуске. Это устраняет воспроизведённый конфликт
фиксированного порта `8010` при `--network host`. Внутренний порт публиковать
наружу не требуется.
При завершении дочернего процесса запись `server_index` в журнале содержит
код завершения `exit_code` и порт `child_port`; они помогают отличить остановку
процесса от ещё не готового индекса.

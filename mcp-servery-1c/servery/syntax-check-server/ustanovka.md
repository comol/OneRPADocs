...
значения `1024` может не хватить. Используйте обычную сеть Docker bridge.
Причина наблюдавшегося отказа полного индекса в режиме `--network host` ещё
исследуется; внутренний дочерний анализатор слушает `127.0.0.1:8010`, но
публиковать порт `8010` наружу не требуется.

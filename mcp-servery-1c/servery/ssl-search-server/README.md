...
**Возврат**: Отформатированная строка с результатами, разделёнными `---`. Заголовок каждого результата содержит базу, версию БСП, `doc_id`, источник и оценку similarity. Если результатов выше порога нет, сервер возвращает типизированный ответ с кодом `not_found`.

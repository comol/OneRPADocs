...
Внутри контейнера сервер и его healthcheck используют фиксированный порт `8008`.
Переменные `MCP_PORT` и `HTTP_PORT` SSLSearchServer не читает. Чтобы подключаться
с хоста через другой порт, измените левую часть проброса Docker, например
`-p 127.0.0.1:18008:8008`, и укажите клиенту `http://localhost:18008/mcp`.
Встроенная проверка `/ready` при этом продолжает обращаться к внутреннему `8008`.
При `--network host` проброса портов нет: сервер занимает порт `8008` самого хоста.
...
Исправление portable-сборки zvec меняет отпечаток полнотекстового индекса.
После установки образа с исправлением сервер автоматически перестраивает старый
индекс из выбранной базы БСП. Это обычная переиндексация SSL: она может повторно
вызвать embedding-провайдер; до завершения `/ready` сообщает о неготовности.
Устанавливать `RESET_DATABASE=true` для этого не требуется.

Нативный FTS старого профиля не используется. Замена одной библиотеки без
перестроения не восстанавливает потерянные совпадения в старых FTS-блоках.

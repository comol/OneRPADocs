...
| Отчёт из конфигурации | Готовый источник базовых метаданных; сохраняется для совместимости и обязателен только при `METADATA_SOURCE=report` | Опционально |
| Проект 1C:EDT | Альтернативный источник метаданных, кода и структуры: базовые метаданные читаются из `.mdo` при `METADATA_SOURCE=auto` (если нет Designer-выгрузки) либо при `METADATA_SOURCE=edt`. Лежащий рядом `*.txt` выгрузку больше не перекрывает. Синтез отчёта из EDT не поддерживается — `METADATA_SOURCE=xml` на EDT-корне отклоняется с `unsupported_by_edt` | Опционально, отдельный отчёт не нужен |
...
Это основной источник для BSL-графа, структуры форм XML, подписок на события, предопределённых элементов, прав ролей и справки. При `METADATA_SOURCE=auto` он же источник базовых метаданных: лежащий рядом `*.txt` его больше не перекрывает.
...
Если политика установки требует использовать именно готовый отчёт, выполните **Конфигурация → Отчёт из конфигурации**, включите все разделы и сохраните `*.txt` в отдельный каталог. Задайте `METADATA_SOURCE=report`. При `auto` выгрузка старше отчёта; при `xml` отчёт намеренно игнорируется.
...
Составной `ТипПараметраКоманды` сохраняется целиком: все типы из XML входят в одно
свойство команды синтезированного отчёта. Изменение правил синтеза обновляет его
отпечаток, поэтому при следующей индексации кэш отчёта будет пересоздан.
{% endhint %}
...
При `LOAD_FORMS_FROM_XML=true` и `LOAD_BSL_SIGNATURES=true` загрузка управляемых
форм ждёт завершения загрузки BSL-графа: обработчики должны существовать до
создания связей `HAS_HANDLER`. После установки beta-сборки, содержащей это исправление, ранее загруженные формы
однократно перечитываются из XML для восстановления пропущенных связей.
Повторный запуск использует обычный resume.

...
| Проект 1C:EDT | Альтернативный источник метаданных, кода и структуры: базовые метаданные читаются из `.mdo`-дескрипторов проекта при `METADATA_SOURCE=auto` (если нет ни `*.txt`, ни Designer-выгрузки) либо при `METADATA_SOURCE=edt`; остальные контуры включаются через `SOURCE_FORMAT_ADAPTERS_ENABLED=true` и `SOURCE_FORMAT=edt`. Синтез текстового отчёта из EDT не поддерживается — `METADATA_SOURCE=xml` на EDT-корне отклоняется с `unsupported_by_edt` | Опционально, отдельный отчёт не нужен |

...
{% hint style="info" %}
Отдельный текстовый отчёт больше не обязателен. При `METADATA_SOURCE=auto` сервер использует его, если он есть; иначе синтезирует совместимый отчёт из Designer XML-выгрузки в `CODE_EXPORT_PATH`; а если нет и её — читает метаданные напрямую из `.mdo`-дескрипторов проекта 1C:EDT.
{% endhint %}
...
| `METADATA_SOURCE` | `auto` — готовый `*.txt`, иначе синтез из Designer XML, иначе дескрипторы проекта 1C:EDT; `report` — требовать отчёт; `xml` — только Designer-выгрузка, отчёт игнорируется, EDT-корень отклоняется; `edt` — только проект 1C:EDT, отчёт не ищется и не пишется | `auto` |
...
Готовый текстовый отчёт для проекта 1C:EDT больше не нужен. При `METADATA_SOURCE=auto` (третья ветка, после `*.txt` и Designer-выгрузки) и при `METADATA_SOURCE=edt` метаданные читаются прямо из `.mdo`-дескрипторов проекта тем же конвейером ingestion, что грузит остальные контуры; отдельного пути и отдельного отчёта для этого нет. Не поддерживается на EDT только *синтез* отчёта: синтезатор читает Designer-раскладку, поэтому `METADATA_SOURCE=xml` на EDT-корне возвращает типизированный отказ `unsupported_by_edt`. Формат EDT для остальных контуров по-прежнему включается через `SOURCE_FORMAT_ADAPTERS_ENABLED=true` и `SOURCE_FORMAT=edt`.

{% hint style="warning" %}
При `METADATA_SOURCE=auto` любой `*.txt` в каталоге отчёта (`METADATA_DIRECTORY`, по умолчанию `/app/metadata`) старше обеих веток выгрузки. Один случайный текстовый файл в этом монтировании молча превращает EDT- или XML-слой в отчётный. Если каталог отчёта не используется, не монтируйте его и не занимайте под что-либо другое.
{% endhint %}
...
- **не занимайте под расширения каталог отчёта `/app/metadata`** — при `METADATA_SOURCE=auto` любой лежащий там `*.txt` выигрывает у обеих веток выгрузки, и слой расширения молча становится отчётным.

...
| `find_form_links` | Связи формы: обработчики, привязки, метаданные, модуль. Владелец может быть указан как `ПереносОтпуска` или `Документ.ПереносОтпуска`. Обработчики событий и команд управляемой формы — процедуры модуля *этой* формы (`Forms/<Форма>/Ext/Form/Module.bsl`), а не одноимённые процедуры других форм владельца |

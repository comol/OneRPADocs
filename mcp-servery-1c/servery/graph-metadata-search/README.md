...
- Понимает связи между объектами (USED_IN, DO_MOVEMENTS_IN, CALLS, ACCESSES_REGISTER, READS_TABLE, EXECUTES, EXTENDS, OVERRIDES)
...
| `get_form_structure` | Полная структура формы: элементы, реквизиты, команды, меню, события. Владельца можно указать с видом (`Документ.РеестрДокументов`); имя, которое носят объекты разных видов с этой формой, без вида отвечает `ambiguous` со списком `candidates` |
| `find_form_links` | Связи формы: обработчики, привязки, метаданные, модуль. Владелец может быть указан как `ПереносОтпуска` или `Документ.ПереносОтпуска`; вид выбирает объект, когда справочник и документ носят одно имя и у каждого своя форма, а голое имя в этом случае отвечает `ambiguous` со списком `candidates` (у каждого `node_id` и полное имя вида `Справочник.РеестрДокументов`). Обработчики событий и команд управляемой формы — процедуры модуля *этой* формы (`Forms/<Форма>/Ext/Form/Module.bsl`), а не одноимённые процедуры других форм владельца |
| `get_access_rights` | Права ролей на объект или на его поле. В режиме каталога расширений — и права ролей слоя (при `LOAD_ROLE_RIGHTS=true`) |
...
| `compare_graph_scope` | Что изменилось между двумя поколениями, проектами или расширением и базой. В режиме `extension_ref` базовая сторона — только объекты базы, связанные с расширением рёбрами `EXTENDS`/`OVERRIDES`, и их связи; слой каталога расширений сравнивается с базой того же проекта без `EXTENSION_BASE_PROJECT` |
...
| `get_graph_stats` | Счётчики графа по типам узлов, связей и evidence. Пуст ли граф — `data.empty`; `data.scope_confined: false` значит, что при выключенном `GRAPH_SCOPE_ENFORCED` счётчики охватывают всю базу; `data.evidence_recorded: false` значит, что evidence не записывается (`REFERENCE_EVIDENCE_ENABLED=false`) и нулевые счётчики evidence и уверенности — норма, а не потеря данных |
...
| Параметр | Тип | По умолчанию | Описание |
|----------|-----|--------------|----------|
| `object_name` | string | — | Имя объекта: `Контрагенты` или с префиксом категории `Справочник.Контрагенты` / `Catalog.Контрагенты` — тогда поиск ограничен этой категорией, и одноимённые объекты других категорий не смешиваются |
| `project_name` | string | — | Фильтр по имени конфигурации |

**Возврат**: Список объектов, ссылающихся на указанный. Постранично — через `max_items`/`cursor`, как у остальных инструментов; читается не больше 25 × `GRAPH_MAX_ITEMS` строк, при обрезке в `warnings` есть предупреждение.
...
| Параметр | Тип | По умолчанию | Описание |
|----------|-----|--------------|----------|
| `object_name` | string | — | Имя объекта; префикс категории (`Справочник.X`, `Catalog.X`) ограничивает поиск этой категорией |
| `project_name` | string | — | Фильтр по имени конфигурации |
| `limit` | int | — | Устарел, не применяется: при передаче в ответе заполняется `deprecated`. Используйте `max_items` |
| `offset` | int | — | Устарел, не применяется. Используйте `cursor` |

**Возврат**: Список реквизитов/измерений/ресурсов с указанием объекта-владельца и типа. Постранично — через `max_items`/`cursor`; читается не больше 25 × `GRAPH_MAX_ITEMS` строк, при обрезке в `warnings` есть предупреждение.
...
| Параметр | Тип | По умолчанию | Описание |
|----------|-----|--------------|----------|
| `register_name` | string | — | Имя регистра: `Продажи` или с префиксом категории `РегистрНакопления.Продажи` — тогда поиск ограничен этой категорией |
| `project_name` | string | — | Фильтр по имени конфигурации |
...
У `trace_impact.direction`, `trace_call_chain.direction` и `search_code.search_type`
допустимые значения перечислены в JSON-схеме MCP (`tools/list`). Значения
чувствительны к регистру; неизвестный вариант отвергается MCP-валидацией до
выполнения инструмента с указанием разрешённых значений. Например,
`trace_call_chain` принимает `callers` или `callees`, но не `both`, а режим
`search_type="vector"` следует задавать как `search_type="semantic"`.
При пропущенном параметре сохраняется значение по умолчанию из таблицы.
...
При вызове `Документы.ИмяОбъекта.Метод()` базовый модуль менеджера имеет
приоритет, только если он объявляет этот метод. Метод, добавленный расширением
в заимствованный модуль менеджера, остаётся доступен в графе вызовов.
После обновления beta от 14 сентября 2026 года ранее пропущенные рёбра `CALLS`
нужно сформировать заново обновлением BSL-графа. Полная штатная перестройка —
`refresh_graph_project` с `mode=full`; на крупном проекте учитывайте стоимость
перестройки и embeddings. Одна замена контейнера существующие рёбра не меняет.
...
**Возврат**: Информация о найденном узле или сообщение об ошибке. Ищутся узлы с индексом по GUID (объекты метаданных, реквизиты, табличные части, формы, ресурсы, измерения); эмбеддинги в свойствах не выводятся.
...
| Параметр | Тип | Описание |
|----------|-----|----------|
| `object_name` | string | Имя объекта метаданных; можно с видом (`Документ.РеестрДокументов`) |
| `extension_name` | string | Имя расширения |

Когда в расширении одно имя носят объекты разных видов, вид в `object_name` выбирает объект. Без вида сравнивается первый по алфавиту вид: заголовок называет его (`Сравнение 'РеестрДокументов' (Документы)`), а следующая строка перечисляет другие объекты с этим именем.
...
Логин: `neo4j`
Пароль: указанный в `NEO4J_PASSWORD` (значения по умолчанию нет — переменную задаёт развёртывание)
...
## Связи BSL-кода (Graph beta от 23.09.2026)

Разбор BSL-модулей выделяет связи, которые раньше терялись:

- **Запись движений.** `Движения.X.Добавить()`, `.Загрузить()`, `.Записать()` и `Движения.X.Записывать = Истина` дают `ACCESSES_REGISTER{access: write, via: movements}` (прежние записи через набор записей — `via: record_set`). Вид регистра из кода не виден, поэтому связь ставится на регистры всех категорий с этим именем, и при нескольких — с `ambiguous: true`.
- **Чтение таблиц запросом.** Нерегистровые источники запросов (`Справочник.`, `Документ.`, табличные части, `Перечисление.`, `ПланВидовХарактеристик.` и т. п.) дают `READS_TABLE`; регистры по-прежнему `ACCESSES_REGISTER{access: read}` с полем `virtual_table`. `find_object_referrers` читает обе связи.
- **Вызовы глобальных модулей.** Вызов без имени модуля разрешается через общие модули с `Глобальный = Истина`: `CALLS{resolution: global_module}`. Локальное объявление в модуле всегда выигрывает.
- **Динамические вызовы.** У процедур появились `dynamic_calls` и `dynamic_call_lines` (`Выполнить(`, `Вычислить(` и обёртки БСП); `trace_call_chain` предупреждает `dynamic_calls_on_chain`, если на цепочке есть такой вызов.
- **Регламентные задания.** `(РегламентноеЗадание)-[:EXECUTES]->(Routine)` по `ИмяМетода`; `trace_call_chain(direction="callers")` показывает задания как точки входа.
- **Модули без исходника** получают `read_status = protected`.

При первом старте нового образа лейны `bsl_call_graph` (версия 3 → 4) и `bsl_register_access` (1 → 2) один раз пересобираются на месте — на ЗУП это минуты. Узлы и embeddings не трогаются, переиндексация и `refresh_graph_project(mode="full")` не нужны.

**Обработчики, заданные строкой (Graph beta от 25.09.2026).** Процедура, которую платформа вызывает по имени из строки, получает вызывающего: `Новый ОписаниеОповещения("Имя", ЭтотОбъект)` — процедура того же модуля, `Новый ОписаниеОповещения("Имя", ОбщийМодуль)` — экспортная процедура этого общего модуля, `ПодключитьОбработчикОжидания("Имя", …)` — процедура того же модуля, иначе глобального общего модуля. Связь — обычный `CALLS` со свойством `via` (`notify_description` или `idle_handler`); если процедура вызывает обработчик ещё и напрямую, связь остаётся прямым вызовом без `via`. `trace_call_chain`, `trace_impact` и поиск неиспользуемых процедур видят такие связи: `…Завершение`-обработчики форм больше не выглядят мёртвым кодом. Не разрешаются и связи не дают имя из переменной, второй аргумент-выражение (`ЭтотОбъект.ВладелецФормы`) и `Форма.ПодключитьОбработчикОжидания(…)`. Лейн `bsl_call_graph` (4 → 5) один раз пересобирается на первом старте, узлы и embeddings не трогаются.

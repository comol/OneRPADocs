...
| `ONEC_AI_WORKSPACE_PATH_MAP` | Соответствия `префикс_на_хосте=корень_в_контейнере`, разделённые `os.pathsep` или переводами строк. Нужны, если агент передаёт абсолютные Windows/UNC-пути; целевой корень обязан входить в `ONEC_AI_WORKSPACE_ROOTS` | *(пусто)* |

...
| `ONEC_AI_TIMEOUT`             | Таймаут отдельного запроса к API (секунды): connect/write/pool                             | `30`                |
| `ONEC_AI_OPERATION_TIMEOUT`   | Единый бюджет времени на всю операцию, включая чтение SSE (секунды)                        | `120`               |
...
| `/release`          | GET   | Идентичность выпуска: версия сервера, digest образа, версия upstream-контракта, идентификатор lock-файла, время сборки |

{% hint style="info" %}
`/health` не делает ни одного обращения к upstream и не раскрывает секретов: это только индикаторы состояния. Идентичность выпуска намеренно вынесена в отдельный `/release`, а метрики сессий содержат счётчики и возрасты, но никогда — идентификаторы.
{% endhint %}
...
Система плагинов в этом сервере не поставляется. Анализ выполняется на стороне сервиса 1С:Напарник, поэтому настройка сводится к тому, как формируется сессия с ним: `MCP_TOOL_CALL_MODE`, `ONEC_AI_SKILL_NAME`, `ONEC_AI_DOC_VERSION`, `ONEC_CONFIG_NAME`, таймауты и лимиты сессий.

Что настраивается вместо плагинов и куда идти, если этого мало: [Серверы без плагинов](../../sistema-pluginov/dorabotka-bez-pluginov.md).

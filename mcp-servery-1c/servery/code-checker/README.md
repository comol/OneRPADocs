...
{% hint style="info" %}
Токен 1С:Напарник доступен **подписчикам ИТС**. Оформить подписку можно на [developer.1c.ru](https://developer.1c.ru/). Если у вас нет подписки, используйте [SyntaxCheckServer](../syntax-check-server/) как альтернативу.
{% endhint %}
...
| Инструмент | Описание |
|------------|----------|
| `check_code` | Проверка кода через 1С:Напарник |
| `analyze_module` | Анализ модуля на соответствие стандартам |
| `get_suggestions` | Получение рекомендаций по улучшению кода |

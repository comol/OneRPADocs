...
В исходниках beta-кандидата пять инструментов с кодом принимают `files`: сервер читает до 20 файлов из смонтированных read-only корней `ONEC_AI_WORKSPACE_ROOTS`, возвращает их SHA-256 в `sources` и не записывает workspace. Это надёжнее передачи большого модуля inline. До публикации нового тега ранее скачанный `latest-beta` может ещё не содержать эту возможность; проверяйте схему инструмента в `tools/list`. См. [Инструменты](instrumenty.md#чтение-исходников-из-workspace-beta) и [Установку](ustanovka.md#beta-с-чтением-исходников-из-workspace).

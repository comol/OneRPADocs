...
В текущем beta-канале пять инструментов с кодом принимают `files`: сервер читает до 20 файлов из смонтированных read-only корней `ONEC_AI_WORKSPACE_ROOTS`, возвращает их SHA-256 в `sources` и не записывает workspace. Это надёжнее передачи большого модуля inline. Абсолютный путь с машины клиента разрешается только через явный `ONEC_AI_WORKSPACE_PATH_MAP`; относительный путь читается от объявленного корня. См. [Инструменты](instrumenty.md#чтение-исходников-из-workspace-beta) и [Установку](ustanovka.md#beta-с-чтением-исходников-из-workspace).

`ONEC_AI_WORKSPACE_PATH_MAP` проверен непосредственно в опубликованном `light-beta`. Настройки повторов upstream-транспорта, присутствующие только в более новом рабочем source, пока не относятся к опубликованному контракту и здесь намеренно не описываются.

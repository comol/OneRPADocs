...
| [1CCodeChecker](code-checker/) | 8007 | Проверка через 1С:Напарник | Низкая | Да (beta) |
...
{% hint style="info" %}
Актуальные версии серверов по умолчанию используют транспорт `streamable-http` на endpoint `/mcp`. Для старых клиентов можно включить SSE через `USESSE=true`; у Graph Metadata Search переменная называется `MCP_USE_SSE`. URL при этом остаётся `/mcp`, если в документации конкретного сервера не указано иное.
{% endhint %}
...
4. **HelpSearchServer** — корпуса справки, спецификаций и стандартов поставляются в образе; папка bin нужна только для своей версии платформы

### Требующие данных

Нужна подготовка данных из конфигурации:

5. **CodeMetadataSearchServer** — нужна выгрузка конфигурации
6. **Graph Metadata Search** — нужна выгрузка + Neo4j

### Требующие внешних ресурсов

7. **1CCodeChecker** — нужен токен 1С:Напарник
...
Часто встречающиеся переменные. Поддержка зависит от сервера; точный список приведён на его странице:

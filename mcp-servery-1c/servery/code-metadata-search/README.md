...
| Параметр | Тип | По умолчанию | Описание |
|----------|-----|--------------|----------|
| `force` | bool | `false` | Если `false` — обновляются только изменённые файлы. Если `true` и есть дорожка с несовместимым FTS (`rebuild_required`), пересобираются только повреждённые дорожки, здоровые переносятся. Если повреждённых нет — полная переиндексация с нуля |
...
| `indexing` | Ход индексации, время последней, статус следующей плановой. `last_outcome` бывает `completed`, `failed`, `cancelled` или `degraded`: `degraded` значит, что прогон завершился, но векторная дорожка не принимает запись (обычно `fts_index_incompatible`). Поиск по опубликованному поколению продолжает отвечать; в `indexing.error` названы дорожки, лечение — `reindex(force=true)` |

...
- CPU/GPU и VRAM либо провайдера API; точную модель и размерность;
...
- Если `stats().indexing.last_outcome` стал `degraded`, а у коллекции `write_health=failed` и `rebuild_required=true`, инкрементальный тик больше не клонирует пустые поколения и не удаляет каталог `<lane>.rebuild`. Поиск по опубликованному поколению продолжает отвечать. Следующий обычный прогон, даже на неизменной выгрузке, восстанавливает дорожку из сохранённых в ней документов без разбора исходников и эмбеддингов; по времени это одно полное слияние дорожки. Если `degraded` остался и после него, лечение — `reindex(force=true)`: из исходников пересобираются только повреждённые дорожки, здоровые (в том числе миллионы чанков `code`) переносятся. Если повреждённых дорожек нет, `force=true` по-прежнему начинает пустое поколение.

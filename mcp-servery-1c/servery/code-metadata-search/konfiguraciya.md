...
{% hint style="warning" %}
**Слияние векторного индекса останавливает запись на всё время слияния.** zvec складывает новые документы во временный плоский буфер и переносит их в HNSW-индекс при оптимизации; периодическая оптимизация запускается каждые `VECTOR_OPTIMIZE_EVERY` документов в фоновом потоке, но вставка в zvec 0.6 сериализована с оптимизацией внутри самого хранилища — поэтому пока идёт слияние, потоки записи ждут, обращений к эмбеддингам нет, а очередь заполняется. Длительность одного слияния растёт с размером индекса и размерностью векторов: на корпусе в миллион фрагментов с 4096-мерными эмбеддингами последние слияния занимают часы, и именно так выглядит «хвост» фазы кода — счётчик файлов стоит, CPU низкий, размер хранилища растёт. Это не зависание: в журнале есть `zvec optimize START … (merging N buffered docs …)`, при первом ожидающем писателе — `zvec add on <lane> waits for the background merge in flight (…)`, по завершении — `zvec optimize DONE … (writers waited N s for it)`, а каждые `INDEX_STALL_HEARTBEAT_SEC` без завершённого файла — строка `<lane> phase: no file completed for N min … merge in flight: …`. Строка прогресса ведёт скорость и ETA по последнему интервалу (`files/min (recent; avg)`), а не по среднему за фазу.
...
Журнал требует дополнительного места: 8 байт на измерение вектора плюс ключи и служебные данные SQLite. Устаревшие записи очищаются при успешной обработке дорожки, а очистка старых поколений сохраняет журнал. Удаление тома или смена проекта лишает восстановления сохранённых данных. Запрос, который провайдер уже обработал, но сервер ещё не сохранил, может повториться; однократная оплата внешнего API не гарантируется.

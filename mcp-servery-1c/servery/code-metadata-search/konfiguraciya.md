...
**Диагностика зависшей структурной дорожки.** Сначала `SUB_INDEX_PROGRESS_WARN_SEC=60` и `SUB_INDEX_PROGRESS_HEARTBEAT_SEC=60` — проход начнёт называть стадию и единицу работы (`source_walk`, `source_attribution`, `origin_purge`, `store_prepare`, `structural_read`, `structural_parse`, `structural_write`, `dependency_graph`, `form_index`, `worker_wait`, `fts_rebuild`, `extension_resolution`, `sub_index_gate`); при `SUB_INDEX_LIVENESS_WITNESS=true` то же самое повторит отдельный процесс. Имя модуля из строки `structural_parse` подставляется в `STRUCTURAL_EXCLUDE`; если таких модулей много, дорожку целиком выключает `INDEX_STRUCTURAL=false`. Один подозрительный модуль измеряется отдельно, без полного прохода: `python src/structural_probe.py <путь>/Module.bsl --json` (по умолчанию таймаут 600 секунд, отключает его только явный `--timeout 0`); печатаются размер, время по стадиям, счётчики и типизированный отказ, исходный текст и абсолютные пути — никогда. После изменения переменных пересоздайте контейнер с сохранением тома данных: `docker restart` новые переменные не применяет.

...
|--------|-------------|---------------------------|------------|------------------------|
| `qwen/qwen3-embedding-8b` | 4096 | инструкция Qwen3 / `document: ` | да | удалённое умолчание поставки |
| `Qwen3-Embedding-4B`, `Qwen3-Embedding-0.6B` (любое имя с `qwen3-embedding-4b` / `-0.6b`) | пробный запрос | инструкция Qwen3 / нет | да | не измерялась |
| Ollama: `qwen3-embedding:8b`, `:4b`, `:0.6b` (любое другое имя с `qwen3-embedding`) | пробный запрос | инструкция Qwen3 / нет | да | не измерялась |
| `embeddinggemma-300m` (в том числе `text-embedding-embeddinggemma-300m-qat` в LM Studio) | 768 | `task: search result \| query: ` / `title: none \| text: ` | да | не измерялась |
| `baai/bge-m3` | 1024 | нет | да | нет: не прошла gate |
| `openai/text-embedding-3-small` | 1536 | нет | да | нет: не прошла gate |

Инструкция Qwen3 для запроса — `Instruct: Given a web search query, retrieve relevant passages that answer the query` и перевод строки перед `Query:`, как в карточке модели. Формулировки под 1С («retrieve the matching 1C code fragment … or configuration metadata object») на том же корпусе оказались хуже (nDCG@10 0,836–0,844 против 0,866, проигрыш на запросах о методах), поэтому у CodeMetadataSearchServer остаётся текст карточки; у остальных серверов инструкция сформулирована под их корпус (см. [Инструкция к запросу у Qwen3](../../embedding-modeli/vybor-modeli.md#инструкция-к-запросу-у-qwen3)). До 22.09.2026 запросы к Qwen3-8B шли с префиксом `query: `, а к 0.6B и 4B — вообще без префикса. На том же корпусе, что и замер 31.08, инструкция подняла у `qwen/qwen3-embedding-8b` MRR с 0,855 до 0,941 и nDCG@10 с 0,733 до 0,858. Префикс документов в замере на качество не повлиял (0,939 / 0,859 с `document: `), поэтому он остался прежним и переиндексация не нужна. Для EmbeddingGemma префикс документов появился впервые: индекс, построенный на этой модели раньше, пересоберите, чтобы старые и новые фрагменты были в одном виде. Префиксы по-прежнему можно переопределить через `EMBEDDING_QUERY_PREFIX` и `EMBEDDING_DOCUMENT_PREFIX`: это нужно для модели, которой нет в таблице.

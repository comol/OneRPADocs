...
В beta исправлена загрузка пресетов Code с FastMCP 2.14: сервер распознаёт зарегистрированные инструменты и их параметры. Если `plugin_state` сообщает для существующего инструмента `It publishes: nothing`, обновите образ и пересоздайте контейнер. Менять файл плагина для этого не требуется.
